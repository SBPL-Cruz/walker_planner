...
#include "planners/rmhaplanner.h"
...
int RMHAPlanner::set_start(int start_stateID)
{
...
int RMHAPlanner::set_goal(int goal_stateID)
{
...
int RMHAPlanner::argmax(std::vector<double>& vec){
    int max_id = 0;
...
int RMHAPlanner::sampleIndex(const std::vector<double>& probs){
    ...
    int* solcost)
{
    ...
    while (!m_open[0].emptyheap() && !time_limit_reached()) {
        ...
        //if( iter % (num_heuristics() - 1) == 0 )
        //    std::cerr<<"Sampled queues: \n";
        //std::cerr<<hidx<<", ";
        //iter++;
...
int RMHAPlanner::force_planning_from_scratch()
{
    return 0;
}

int RMHAPlanner::force_planning_from_scratch_and_free_memory()
{
    return 0;
}

void RMHAPlanner::costs_changed(StateChangeQuery const & stateChange)
{
}

void RMHAPlanner::costs_changed()
{
}

int RMHAPlanner::set_search_mode(bool bSearchUntilFirstSolution)
{
    return m_params.return_first_solution = bSearchUntilFirstSolution;
}

void RMHAPlanner::set_initialsolution_eps(double eps)
{
    m_params.initial_eps = eps;
}

double RMHAPlanner::get_initial_eps()
{
    return m_params.initial_eps;
}

double RMHAPlanner::get_solution_eps() const
{
    return m_eps_satisfied;
}

double RMHAPlanner::get_final_epsilon()
{
    return m_eps_satisfied;
}

double RMHAPlanner::get_final_eps_planning_time()
{
    return m_elapsed;
}

double RMHAPlanner::get_initial_eps_planning_time()
{
    return m_elapsed;
}

int RMHAPlanner::get_n_expands() const
{
    return m_num_expansions;
}

int RMHAPlanner::get_n_expands_init_solution()
{
    return m_num_expansions;
}

void RMHAPlanner::get_search_stats(std::vector<PlannerStats>* s)
{
}

void RMHAPlanner::set_initial_mha_eps(double eps)
{
    m_initial_eps_mha = eps;
}

void RMHAPlanner::set_final_eps(double eps)
{
    m_params.final_eps = eps;
}

void RMHAPlanner::set_dec_eps(double eps)
{
    m_params.dec_eps = eps;
}

void RMHAPlanner::set_max_expansions(int expansion_count)
{
    m_max_expansions = expansion_count;
}

void RMHAPlanner::set_max_time(double max_time)
{
    m_params.max_time = max_time;
}

double RMHAPlanner::get_initial_mha_eps() const
{
    return m_initial_eps_mha;
}

double RMHAPlanner::get_final_eps() const
{
    return m_params.final_eps;
}

double RMHAPlanner::get_dec_eps() const
{
    return m_params.dec_eps;
}

int RMHAPlanner::get_max_expansions() const
{
    return m_max_expansions;
}

double RMHAPlanner::get_max_time() const
{
    return m_params.max_time;
}

int RMHAPlanner::get_num_expansions() const {
    return m_num_expansions;
}

bool RMHAPlanner::check_params(const ReplanParams& params)
{
...
bool RMHAPlanner::time_limit_reached() const
{
...
MHASearchState* RMHAPlanner::get_state(int state_id)
{
...
void RMHAPlanner::reinit_state(MHASearchState* state)
{
...
void RMHAPlanner::reinit_search()
{
    clear_open_lists();
}

void RMHAPlanner::clear_open_lists()
{
    for (int i = 0; i < num_heuristics(); ++i) {
...
int RMHAPlanner::compute_key(MHASearchState* state, int hidx)
{
    //if(hidx == 0){
...
void RMHAPlanner::expand(MHASearchState* state, int hidx)
{
    //std::cerr<<"hid: "<<hidx<<"\n";
...
MHASearchState* RMHAPlanner::state_from_open_state(
...
int RMHAPlanner::compute_heuristic(int state_id, int hidx)
{
    if (hidx == 0) {
...
int RMHAPlanner::get_minf(CHeap& pq) const
{
    return pq.getminkeyheap().key[0];
}

void RMHAPlanner::insert_or_update(MHASearchState* state, int hidx, int f)
{
    CKey new_key;
    new_key.key[0] = f;
...
void RMHAPlanner::extract_path(std::vector<int>* solution_path, int* solcost)
{
...
bool RMHAPlanner::closed_in_anc_search(MHASearchState* state) const
{
    return state->closed_in_anc;
}

bool RMHAPlanner::closed_in_add_search(MHASearchState* state) const
{
    return state->closed_in_add;
}

bool RMHAPlanner::closed_in_any_search(MHASearchState* state) const
{
