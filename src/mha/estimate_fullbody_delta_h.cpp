...
int main(int argc, char** argv) {
    ...
    if(!constructHeuristicsBase( robot_heurs, rep_ids, bfs_heurs, space.get(), grid_ptr.get(), rm.get(), planning_config )){
    ...
    while(ep <= std::min(planning_config.end_planning_episode, read_from_file.numEpisodes() - 1))
    {
        ...
        auto start = read_from_file.getStart(ep);
        space->setGoal(goal);
        for(int i = 0; i < space->numGoalBasePoses(); i++)
        {
            auto base_goal = space->getGoalBasePose(i);
            goal.pose.translation().x() = base_goal[0];
            goal.pose.translation().y() = base_goal[1];
            goal.pose.translation().z() = 0.7;
            goal.xyz_tolerance[0] = 0.5;
            goal.xyz_tolerance[1] = 0.5;
            goal.xyz_tolerance[2] = 0.5;
            goal.xyz_tolerance[0] = 6.28;
            goal.xyz_tolerance[1] = 6.28;
            goal.xyz_tolerance[2] = 6.28;
            std::stringstream ss;
            //for(int j = 0; j < start.size(); j++)
                //ss<< start[j]<< " ";
            //ROS_ERROR_STREAM(ss.str());
            //if(cc.isStateValid(start, true))
            auto robotstate_start = start.joint_state.position;
            mplanner_ros.addStartGoal(robotstate_start, goal);
            starts.push_back(robotstate_start);
    ...
    */
    ...
    std::string delta_h_file_name = "base_delta_h.txt"; //+ instance + ".txt";
    ...
    while(ep < mplanner_ros.numEpisodes())
    {
        ...
        if(status != ExecutionStatus::WAITING)
        {
            ...
            double h_drop = (1.0 * search_ptr->m_initial_h[1] - search_ptr->m_best_h[1]);
            int expansions = search_ptr->m_inad_expansions;
            ROS_ERROR("h drop : %f, expansions: %d", h_drop, expansions);
            delta_h_file<< h_drop<< " "<< expansions<< "\n";
            delta_h_file.close();
        }
