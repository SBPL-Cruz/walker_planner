...
int main(int argc, char** argv){
    ros::init(argc, argv, "rmha_planner");
    ros::NodeHandle nh;
    ros::NodeHandle ph("~");
    ...
    auto scheduling_policy = std::make_unique<UniformlyRandomPolicy>(inad_heurs.size(), time(NULL));

    using MotionPlanner = MPlanner::MotionPlanner<RMHAPlanner, smpl::ManipLattice>;
    auto search_ptr = std::make_unique<RMHAPlanner>( space.get(), anchor_heur, inad_heurs.data(), inad_heurs.size(), scheduling_policy.get() );
