...
// Slit in the walls at the doors
std::vector<moveit_msgs::CollisionObject> GetMultiRoomMap2CollisionCubes(
    ...
    pose.position.x = alley_left_wall_x;
    pose.position.y = ((map_third_y/2) - d)/2;
    pose.position.z = 0.3;//z;
    dims[0] = th;
    dims[1] = map_third_y/2 - d + 0.05;
    dims[2] = 0.6;//ht;
    objs.push_back(GetCollisionCube(pose, dims, frame_id, object_id));
...
std::vector<moveit_msgs::CollisionObject> GetTablesMapCollisionCubes(
    ...
    const double max_object_size = 0.25;
    const double min_object_size = 0.10;
    ...
    for(int i = 0; i < config.n_tables; i++){
        ...
        while(!no_overlap){
            ...
            pose.position.x = getRandNum( config.max_table_len/2 + offset, config.x_max - config.max_table_len/2 - offset );
            pose.position.y = getRandNum( config.max_table_len/2 + offset, config.y_max - config.max_table_len/2 - offset );
            if(minDistFromEach(std::make_pair(pose.position.x, pose.position.y),
                        table_locs) > config.min_dist_bw_tables)
                no_overlap = true;
        }
        ...
        geometry_msgs::Pose obj_pose;
        std::vector<double> obj_dims(3,0);
        for(int j = 0; j < config.n_objects_per_table; j++){
            object_id = "object" + std::to_string(i) + std::to_string(j);
            obj_dims[0] = getRandNum( min_object_size/1.5, max_object_size/1.5 );
            obj_dims[1] = getRandNum( min_object_size/1.5, max_object_size/1.5 );
            obj_dims[2] = getRandNum( min_object_size, max_object_size );
            obj_pose.position.x = getRandNum(pose.position.x - dims[0]/2 + obj_dims[0]/2, pose.position.x + dims[0]/2 - obj_dims[0]/2);
            obj_pose.position.y = getRandNum(pose.position.y - dims[1]/2 + obj_dims[1]/2, pose.position.y + dims[1]/2 - obj_dims[1]/2);
            obj_pose.position.z = dims[2] + obj_dims[2]/2;
    ...
    // lower
    object_id ="wall1";
    pose.position.x = x_max/2;
    pose.position.y = thickness/2;
    ...
    object_id ="wall2";
    pose.position.x = x_max/2;
    pose.position.y = y_max - thickness/2;
    pose.position.z = height/2;
    dims[0] = x_max;
    dims[1] = thickness;
    dims[2] = height;
    ...
    // left
    object_id ="wall3";
    pose.position.x = thickness/2;
    pose.position.y = y_max/2;
    pose.position.z = height/2;
    dims[0] = thickness;
    dims[1] = y_max;
    dims[2] = height;
    ...
    // right
    object_id ="wall4";
    pose.position.x = x_max - thickness/2;
    pose.position.y = y_max/2;
    pose.position.z = height/2;
    dims[0] = thickness;
    dims[1] = y_max;
