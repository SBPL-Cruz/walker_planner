...
MobManipDiscreteFeatures<6>::MobManipDiscreteFeatures(
        smpl::KDLRobotModel* _rm,
        smpl::ManipLattice* _manip_lattice,
        smpl::Bfs3DBaseHeuristic* _bfs_3d_base1,
        smpl::Bfs3DBaseHeuristic* _bfs_3d_base2,
        smpl::Bfs3DHeuristic* _bfs_3d) :
    AbstractContext<6, int>(),
    m_rm{_rm}, m_env{_manip_lattice}, m_bfs_3d_base1{_bfs_3d_base1},
    m_bfs_3d_base2{_bfs_3d_base2}, m_bfs_3d{_bfs_3d}
{

}

auto MobManipDiscreteFeatures<6>::getContext( int _state_id )
    -> ContextArray
{
    //ROS_DEBUG_NAMED(LOG, "getContext for %d", _state_id);
    ContextArray context;
    ...
    // For obstacles
    context[0] = m_bfs_3d->GetGoalHeuristic(_state_id) / cost_per_cell;
    //ROS_DEBUG_NAMED(LOG, "  BFS3D");
    ...
    // 1 = Base distance to goal
    //auto goal_base1 = m_env->getGoalBasePose(0);
    //auto goal_base2 = m_env->getGoalBasePose(1);
    ...
    // BFS3DBase
    context[1] = m_bfs_3d_base1->GetGoalHeuristic(_state_id) / cost_per_cell;
    //ROS_DEBUG_NAMED(LOG, "  BFS3DBase1");
    context[2] = m_bfs_3d_base2->GetGoalHeuristic(_state_id) / cost_per_cell;
    //ROS_DEBUG_NAMED(LOG, "  BFS3DBase2");

    // Dist between base yaw and goal yaw
    double y, p, r;
    smpl::angles::get_euler_zyx(goal_rot, y, p, r);
    double theta_res = m_env->resolutions()[2];
    context[3] = smpl::angles::shortest_angle_dist(robot_state[2], y) / theta_res;
    //ROS_DEBUG_NAMED(LOG, "  Yaw");
    ...
    context[4] = circum_radius;
    //ROS_DEBUG_NAMED(LOG, "  Circumradius");

    // Find BFS3D_Base path and compute distance to the nearest narrow passage.
    auto path_to_goal = m_bfs_3d_base1->getPathToGoal(robot_state);
    // Ignore narrow passages beyond 1.5m.
    const int MAX_HORIZON = 30;
    context[5] = 0;
    for(int i = 0; i < std::min(MAX_HORIZON, (int) path_to_goal.size()); i++)
    ...
    //ROS_DEBUG_NAMED(LOG, "  Narrow Passage");
    ...
    ROS_DEBUG_NAMED(LOG, "Context for state: %d", _state_id);
    //ROS_DEBUG_NAMED(LOG, "  3D-base  3D   Circum-radius  Dist-to-narrow-passage");
    //ROS_DEBUG_NAMED(LOG, "    Dist to goal  BFS-End-Eff      Yaw dist to goal    Dist-to-narrow-passage\n");
    ROS_DEBUG_NAMED(LOG, "  %d,  %d,  %d,  %d,  %d,  %d", context[0], context[1], context[2], context[3], context[4], context[5]);
...
void MobManipDiscreteFeatures<6>::setRobotArmLength( double _len )
{
    m_arm_len = _len;
}
